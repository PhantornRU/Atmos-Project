...
[SelectionBase]
public class TileGas : MonoBehaviour
{
    [Header("Параметры")]
    ...
    public void PressureTransmission(float tick_time)
    {
        if (isActive)
        {
            countActive = 0; //счет не возможных передач
            for (int num = 0; num < massGas.Length; num++) //для оптимизации, вместо foreach(TileGas _tileGas in massGas) - использую For, также поэтому массивы, а не списки List
            {
                if (massGas[num] != null && pressure > massGas[num].pressure + gasDiff)
                {
                    ...
                    speedChange = 1.414f * Mathf.Abs(pressure - massGas[num].pressure) * tick_time * Time.deltaTime;
                    ...
                    {
                        //смещение силой
                        Vector2 directionToTile = ((Vector2)massGas[num].transform.position - body.position).normalized;
                        float distance = ((Vector2)transform.position - body.position).magnitude;
                        float strength =  speedChange * Mathf.Abs(pressure - massGas[num].pressure) * distance / ( body.mass);
            ...
            if (countActive == 0)
            {   //!!!некорректно работает, нужно перенастроить, актив не вырубается при завершении!!!
                isActive = false;
            }
        }
    }
    ...
    {
        if (isNeedSmoke && CheckGasBlock(tilePlace.x, tilePlace.y) == false)
        {
    ...
    ///</summary>
    private void ActivateNearTiles()
    {
        int num = 0;
